class HaikuGenerator
  ...
  def self.generate(phrase:)
    ...
    Natto::MeCab.new.parse(phrase) do |mecab_node|
      unless mecab_node.surface != "。" && mecab_node.feature.split(",").first == "記号" || mecab_node.is_eos? || mecab_node.feature.split(",").last == "*"
        hash_map << {
          pos: mecab_node.feature.split(",").first,
          letters: mecab_node.feature.split(",").last.gsub(/[ゃ]|[ゅ]|[ょ]|[ゎ][ぇ]|[ヮ]|[ャ]|[ョ]|[ュ]|[ェ]/, "").length,
    ...
    hash_map.each.with_index do |_, i|
      ...
      hash_map.each.with_index do |_, j|
        if hash_map[i+j].nil? || hash_map[i+j][:surface] == "。"
          is_break = true
        end
        break if is_break
        ...
        if count == 5
          if !ALLOWED_POS_MAP.include?(temp[0][:pos]) || temp[-1][:pos] == "接頭詞" || temp[-1][:surface][-1] == "っ"
            is_break = true and break
          end
          temp2 << temp
          temp = []
          hash_map.each.with_index do |_, k|
            if hash_map[i+j+k+1].nil? || hash_map[i+j+k+1][:surface] == "。"
              is_break = true
            end
            break if is_break
            ...
            if count == 12
              ...
              hash_map.each.with_index do |_, l|
                if hash_map[i+j+k+l+2].nil? || hash_map[i+j+k+l+2][:surface] == "。"
                  is_break = true
                end
                break if is_break
                count += hash_map[i+j+k+l+2][:letters]
                temp << { surface: hash_map[i+j+k+l+2][:surface], pos: hash_map[i+j+k+l+2][:pos] }
                if count == 17
                  if !ALLOWED_POS_MAP.include?(temp[0][:pos]) || temp[-1][:pos] == "接頭詞" || temp[-1][:surface][-1] == "っ"
                    is_break = true and break
                  end
